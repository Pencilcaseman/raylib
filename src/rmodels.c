...
#include "raymath.h"        // Required for: RlVector3, RlQuaternion and RlMatrix functionality
...
static RlModel LoadOBJ(const char *fileName);     // Load OBJ mesh data
#endif
#if defined(SUPPORT_FILEFORMAT_IQM)
static RlModel LoadIQM(const char *fileName);     // Load IQM mesh data
static RlModelAnimation *LoadModelAnimationsIQM(const char *fileName, unsigned int *animCount);   // Load IQM animation data
#endif
#if defined(SUPPORT_FILEFORMAT_GLTF)
static RlModel LoadGLTF(const char *fileName);    // Load GLTF mesh data
static RlModelAnimation *LoadModelAnimationsGLTF(const char *fileName, unsigned int *animCount);  // Load GLTF animation data
#endif
#if defined(SUPPORT_FILEFORMAT_VOX)
static RlModel LoadVOX(const char *filename);     // Load VOX mesh data
#endif
#if defined(SUPPORT_FILEFORMAT_M3D)
static RlModel LoadM3D(const char *filename);     // Load M3D mesh data
static RlModelAnimation *LoadModelAnimationsM3D(const char *fileName, unsigned int *animCount);   // Load M3D animation data
#endif
#if defined(SUPPORT_FILEFORMAT_OBJ) || defined(SUPPORT_FILEFORMAT_MTL)
static void ProcessMaterialsOBJ(RlMaterial *rayMaterials, tinyobj_material_t *materials, int materialCount);  // Process obj materials
#endif
...
void RL_DrawLine3D(RlVector3 startPos, RlVector3 endPos, RlColor color)
{
    rlBegin(RLGL_LINES);
...
void RL_DrawPoint3D(RlVector3 position, RlColor color)
{
    rlPushMatrix();
...
void RL_DrawCircle3D(RlVector3 center, float radius, RlVector3 rotationAxis, float rotationAngle, RlColor color)
{
    rlPushMatrix();
...
void RL_DrawTriangle3D(RlVector3 v1, RlVector3 v2, RlVector3 v3, RlColor color)
{
...
void RL_DrawTriangleStrip3D(RlVector3 *points, int pointCount, RlColor color)
{
...
void RL_DrawCube(RlVector3 position, float width, float height, float length, RlColor color)
{
    float x = 0.0f;
    float y = 0.0f;
    float z = 0.0f;
...
void RL_DrawCubeV(RlVector3 position, RlVector3 size, RlColor color)
{
    RL_DrawCube(position, size.x, size.y, size.z, color);
}

// Draw cube wires
void RL_DrawCubeWires(RlVector3 position, float width, float height, float length, RlColor color)
{
    float x = 0.0f;
    float y = 0.0f;
    float z = 0.0f;
...
void RL_DrawCubeWiresV(RlVector3 position, RlVector3 size, RlColor color)
{
    RL_DrawCubeWires(position, size.x, size.y, size.z, color);
}

// Draw sphere
void RL_DrawSphere(RlVector3 centerPos, float radius, RlColor color)
{
    RL_DrawSphereEx(centerPos, radius, 16, 16, color);
}

// Draw sphere with extended parameters
void RL_DrawSphereEx(RlVector3 centerPos, float radius, int rings, int slices, RlColor color)
{
    rlPushMatrix();
...
// Draw sphere wires
void RL_DrawSphereWires(RlVector3 centerPos, float radius, int rings, int slices, RlColor color)
{
    rlPushMatrix();
        // NOTE: Transformation is applied in inverse order (scale -> translate)
        rlTranslatef(centerPos.x, centerPos.y, centerPos.z);
...
void RL_DrawCylinder(RlVector3 position, float radiusTop, float radiusBottom, float height, int sides, RlColor color)
{
    if (sides < 3) sides = 3;
...
// NOTE: It could be also used for pyramid and cone
void RL_DrawCylinderEx(RlVector3 startPos, RlVector3 endPos, float startRadius, float endRadius, int sides, RlColor color)
{
    if (sides < 3) sides = 3;

    RlVector3 direction = { endPos.x - startPos.x, endPos.y - startPos.y, endPos.z - startPos.z };
    if ((direction.x == 0) && (direction.y == 0) && (direction.z == 0)) return;

    // Construct a basis of the base and the top face:
    RlVector3 b1 = Vector3Normalize(Vector3Perpendicular(direction));
    RlVector3 b2 = Vector3Normalize(Vector3CrossProduct(b1, direction));
    ...
    rlBegin(RLGL_TRIANGLES);
        ...
        for (int i = 0; i < sides; i++) {
            ...
            RlVector3 w1 = { startPos.x + s1*b1.x + c1*b2.x, startPos.y + s1*b1.y + c1*b2.y, startPos.z + s1*b1.z + c1*b2.z };
            float s2 = sinf(baseAngle*(i + 1))*startRadius;
            float c2 = cosf(baseAngle*(i + 1))*startRadius;
            RlVector3 w2 = { startPos.x + s2*b1.x + c2*b2.x, startPos.y + s2*b1.y + c2*b2.y, startPos.z + s2*b1.z + c2*b2.z };
            float s3 = sinf(baseAngle*(i + 0))*endRadius;
            float c3 = cosf(baseAngle*(i + 0))*endRadius;
            RlVector3 w3 = { endPos.x + s3*b1.x + c3*b2.x, endPos.y + s3*b1.y + c3*b2.y, endPos.z + s3*b1.z + c3*b2.z };
            float s4 = sinf(baseAngle*(i + 1))*endRadius;
            float c4 = cosf(baseAngle*(i + 1))*endRadius;
            RlVector3 w4 = { endPos.x + s4*b1.x + c4*b2.x, endPos.y + s4*b1.y + c4*b2.y, endPos.z + s4*b1.z + c4*b2.z };
...
// NOTE: It could be also used for pyramid and cone
void RL_DrawCylinderWires(RlVector3 position, float radiusTop, float radiusBottom, float height, int sides, RlColor color)
{
    if (sides < 3) sides = 3;
...
// NOTE: It could be also used for pyramid and cone
void RL_DrawCylinderWiresEx(RlVector3 startPos, RlVector3 endPos, float startRadius, float endRadius, int sides, RlColor color)
{
    if (sides < 3) sides = 3;
...
void RL_DrawCapsule(RlVector3 startPos, RlVector3 endPos, float radius, int slices, int rings, RlColor color)
{
    ...
    if (sphereCase) direction = (RlVector3){0.0f, 1.0f, 0.0f};

    // Construct a basis of the base and the caps:
    RlVector3 b0 = Vector3Normalize(direction);
    RlVector3 b1 = Vector3Normalize(Vector3Perpendicular(direction));
    RlVector3 b2 = Vector3Normalize(Vector3CrossProduct(b1, direction));
    RlVector3 capCenter = endPos;
...
void RL_DrawCapsuleWires(RlVector3 startPos, RlVector3 endPos, float radius, int slices, int rings, RlColor color)
{
    if (slices < 3) slices = 3;
...
// Draw a plane
void RL_DrawPlane(RlVector3 centerPos, RlVector2 size, RlColor color)
{
...
// Draw a ray line
void RL_DrawRay(RlRay ray, RlColor color)
{
    float scale = 10000;
...
RlModel RL_LoadModel(const char *fileName)
{
    RlModel model = { 0 };
...
#if defined(SUPPORT_MESH_GENERATION)
        TRACELOG(RL_LOG_WARNING, "MESH: [%s] Failed to load mesh data, default to cube mesh", fileName);
        model.meshes[0] = RL_GenMeshCube(1.0f, 1.0f, 1.0f);
#else
        TRACELOG(RL_LOG_WARNING, "MESH: [%s] Failed to load mesh data", fileName);
#endif
    }
    else
    {
    ...
    {
        TRACELOG(RL_LOG_WARNING, "MATERIAL: [%s] Failed to load material data, default to white material", fileName);
...
RlModel RL_LoadModelFromMesh(RlMesh mesh)
{
    RlModel model = { 0 };
...
bool RL_IsModelReady(RlModel model)
{
...
void RL_UnloadModel(RlModel model)
{
    // Unload meshes
    ...
    TRACELOG(RL_LOG_INFO, "MODEL: Unloaded model (and meshes) from RAM and VRAM");
}

// Compute model bounding box limits (considers all meshes)
RlBoundingBox RL_GetModelBoundingBox(RlModel model)
{
    RlBoundingBox bounds = { 0 };
    ...
    if (model.meshCount > 0)
    {
        ...
        {
            RlBoundingBox tempBounds = RL_GetMeshBoundingBox(model.meshes[i]);
...
void RL_UploadMesh(RlMesh *mesh, bool dynamic)
{
    if (mesh->vaoId > 0)
    {
        // Check if mesh has already been loaded in GPU
        TRACELOG(RL_LOG_WARNING, "VAO: [ID %i] Trying to re-load an already loaded mesh", mesh->vaoId);
        return;
    }
    ...
    if (mesh->normals != NULL)
    {
        ...
        rlSetVertexAttribute(2, 3, RLGL_FLOAT, 0, 0, 0);
    ...
    if (mesh->colors != NULL)
    {
        ...
        rlSetVertexAttribute(3, 4, RLGL_UNSIGNED_BYTE, 1, 0, 0);
    ...
    if (mesh->tangents != NULL)
    {
        ...
        rlSetVertexAttribute(4, 4, RLGL_FLOAT, 0, 0, 0);
    ...
    if (mesh->texcoords2 != NULL)
    {
        ...
        rlSetVertexAttribute(5, 2, RLGL_FLOAT, 0, 0, 0);
    ...
    else
    {
        ...
        rlSetVertexAttributeDefault(5, value, RL_SHADER_ATTRIB_VEC2, 2);
    ...
    if (mesh->vaoId > 0) TRACELOG(RL_LOG_INFO, "VAO: [ID %i] RlMesh uploaded successfully to VRAM (GPU)", mesh->vaoId);
    else TRACELOG(RL_LOG_INFO, "VBO: RlMesh uploaded successfully to VRAM (GPU)");
...
void RL_UpdateMeshBuffer(RlMesh mesh, int index, const void *data, int dataSize, int offset)
{
    rlUpdateVertexBuffer(mesh.vboId[index], data, dataSize, offset);
}

// Draw a 3d mesh with material and transform
void RL_DrawMesh(RlMesh mesh, RlMaterial material, RlMatrix transform)
{
    ...
    RlMatrix matModel = MatrixIdentity();
    RlMatrix matView = rlGetMatrixModelview();
    RlMatrix matModelView = MatrixIdentity();
    RlMatrix matProjection = rlGetMatrixProjection();

    // Upload view and projection matrices (if locations available)
    if (material.shader.locs[RL_SHADER_LOC_MATRIX_VIEW] != -1) rlSetUniformMatrix(material.shader.locs[RL_SHADER_LOC_MATRIX_VIEW], matView);
    if (material.shader.locs[RL_SHADER_LOC_MATRIX_PROJECTION] != -1) rlSetUniformMatrix(material.shader.locs[RL_SHADER_LOC_MATRIX_PROJECTION], matProjection);

    // RlModel transformation matrix is sent to shader uniform location: RL_SHADER_LOC_MATRIX_MODEL
    if (material.shader.locs[RL_SHADER_LOC_MATRIX_MODEL] != -1) rlSetUniformMatrix(material.shader.locs[RL_SHADER_LOC_MATRIX_MODEL], transform);
    ...
    if (material.shader.locs[RL_SHADER_LOC_MATRIX_NORMAL] != -1) rlSetUniformMatrix(material.shader.locs[RL_SHADER_LOC_MATRIX_NORMAL], MatrixTranspose(MatrixInvert(matModel)));
    //-----------------------------------------------------
    ...
    for (int i = 0; i < MAX_MATERIAL_MAPS; i++)
    {
        if (material.maps[i].texture.id > 0)
        {
            ...
            if ((i == RL_MATERIAL_MAP_IRRADIANCE) ||
                (i == RL_MATERIAL_MAP_PREFILTER) ||
                (i == RL_MATERIAL_MAP_CUBEMAP)) rlEnableTextureCubemap(material.maps[i].texture.id);
            else rlEnableTexture(material.maps[i].texture.id);

            rlSetUniform(material.shader.locs[RL_SHADER_LOC_MAP_DIFFUSE + i], &i, RL_SHADER_UNIFORM_INT, 1);
        }
    }
    ...
    if (!rlEnableVertexArray(mesh.vaoId))
    {
        ...
        rlSetVertexAttribute(material.shader.locs[RL_SHADER_LOC_VERTEX_POSITION], 3, RLGL_FLOAT, 0, 0, 0);
        rlEnableVertexAttribute(material.shader.locs[RL_SHADER_LOC_VERTEX_POSITION]);

        // Bind mesh VBO data: vertex texcoords (shader-location = 1)
        rlEnableVertexBuffer(mesh.vboId[1]);
        rlSetVertexAttribute(material.shader.locs[RL_SHADER_LOC_VERTEX_TEXCOORD01], 2, RLGL_FLOAT, 0, 0, 0);
        ...
        if (material.shader.locs[RL_SHADER_LOC_VERTEX_NORMAL] != -1)
        {
            ...
            rlSetVertexAttribute(material.shader.locs[RL_SHADER_LOC_VERTEX_NORMAL], 3, RLGL_FLOAT, 0, 0, 0);
        ...
        if (material.shader.locs[RL_SHADER_LOC_VERTEX_COLOR] != -1)
        {
            if (mesh.vboId[3] != 0)
            {
                ...
                rlSetVertexAttribute(material.shader.locs[RL_SHADER_LOC_VERTEX_COLOR], 4, RLGL_UNSIGNED_BYTE, 1, 0, 0);
        ...
        if (material.shader.locs[RL_SHADER_LOC_VERTEX_TANGENT] != -1)
        {
            ...
            rlSetVertexAttribute(material.shader.locs[RL_SHADER_LOC_VERTEX_TANGENT], 4, RLGL_FLOAT, 0, 0, 0);
        ...
        if (material.shader.locs[RL_SHADER_LOC_VERTEX_TEXCOORD02] != -1)
        {
            ...
            rlSetVertexAttribute(material.shader.locs[RL_SHADER_LOC_VERTEX_TEXCOORD02], 2, RLGL_FLOAT, 0, 0, 0);
    ...
    for (int eye = 0; eye < eyeCount; eye++)
    {
        ...
        RlMatrix matModelViewProjection = MatrixIdentity();
    ...
    for (int i = 0; i < MAX_MATERIAL_MAPS; i++)
    {
        if (material.maps[i].texture.id > 0)
        {
            ...
            if ((i == RL_MATERIAL_MAP_IRRADIANCE) ||
                (i == RL_MATERIAL_MAP_PREFILTER) ||
                (i == RL_MATERIAL_MAP_CUBEMAP)) rlDisableTextureCubemap();
...
void RL_DrawMeshInstanced(RlMesh mesh, RlMaterial material, const RlMatrix *transforms, int instances)
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    ...
    // Instances transformation matrices are send to shader attribute location: RL_SHADER_LOC_MATRIX_MODEL
    for (unsigned int i = 0; i < 4; i++)
    {
        rlEnableVertexAttribute(material.shader.locs[RL_SHADER_LOC_MATRIX_MODEL] + i);
        rlSetVertexAttribute(material.shader.locs[RL_SHADER_LOC_MATRIX_MODEL] + i, 4, RLGL_FLOAT, 0, sizeof(RlMatrix), (void *)(i*sizeof(RlVector4)));
...
void RL_UnloadMesh(RlMesh mesh)
{
...
bool RL_ExportMesh(RlMesh mesh, const char *fileName)
{
    bool success = false;
    ...
    if (RL_IsFileExtension(fileName, ".obj"))
    {
        ...
        byteCount += sprintf(txtData + byteCount, "# //                                                                              //\n");
        byteCount += sprintf(txtData + byteCount, "# // rMeshOBJ exporter v1.0 - RlMesh exported as triangle faces and not optimized   //\n");
        byteCount += sprintf(txtData + byteCount, "# //                                                                              //\n");
...
static void ProcessMaterialsOBJ(RlMaterial *materials, tinyobj_material_t *mats, int materialCount)
{
    ...
    for (int m = 0; m < materialCount; m++)
    {
        ...
        materials[m].maps[MATERIAL_MAP_DIFFUSE].texture = (RlTexture2D){ rlGetTextureIdDefault(), 1, 1, 1, RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8 };
        ...
        else materials[m].maps[MATERIAL_MAP_DIFFUSE].color = (RlColor){ (unsigned char)(mats[m].diffuse[0]*255.0f), (unsigned char)(mats[m].diffuse[1]*255.0f), (unsigned char)(mats[m].diffuse[2] * 255.0f), 255 }; //float diffuse[3];
        ...
        materials[m].maps[MATERIAL_MAP_SPECULAR].color = (RlColor){ (unsigned char)(mats[m].specular[0]*255.0f), (unsigned char)(mats[m].specular[1]*255.0f), (unsigned char)(mats[m].specular[2] * 255.0f), 255 }; //float specular[3];
        materials[m].maps[MATERIAL_MAP_SPECULAR].value = 0.0f;

        if (mats[m].bump_texname != NULL) materials[m].maps[RL_MATERIAL_MAP_NORMAL].texture = RL_LoadTexture(mats[m].bump_texname);  //char *bump_texname; // map_bump, bump
        materials[m].maps[RL_MATERIAL_MAP_NORMAL].color = WHITE;
        materials[m].maps[RL_MATERIAL_MAP_NORMAL].value = mats[m].shininess;

        materials[m].maps[RL_MATERIAL_MAP_EMISSION].color = (RlColor){ (unsigned char)(mats[m].emission[0]*255.0f), (unsigned char)(mats[m].emission[1]*255.0f), (unsigned char)(mats[m].emission[2] * 255.0f), 255 }; //float emission[3];

        if (mats[m].displacement_texname != NULL) materials[m].maps[RL_MATERIAL_MAP_HEIGHT].texture = RL_LoadTexture(mats[m].displacement_texname);  //char *displacement_texname; // disp
    }
}
#endif

// Load materials from model file
RlMaterial *RL_LoadMaterials(const char *fileName, int *materialCount)
{
    RlMaterial *materials = NULL;
    ...
    if (RL_IsFileExtension(fileName, ".mtl"))
    {
        ...
        if (result != TINYOBJ_SUCCESS) TRACELOG(RL_LOG_WARNING, "MATERIAL: [%s] Failed to parse materials file", fileName);

        materials = RL_MemAlloc(count*sizeof(RlMaterial));
        ProcessMaterialsOBJ(materials, mats, count);

        tinyobj_materials_free(mats, count);
    }
#else
    TRACELOG(RL_LOG_WARNING, "FILEIO: [%s] Failed to load material file", fileName);
#endif
...
RlMaterial RL_LoadMaterialDefault(void)
{
    ...
    material.maps[MATERIAL_MAP_DIFFUSE].texture = (RlTexture2D){ rlGetTextureIdDefault(), 1, 1, 1, RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8 };
    //material.maps[RL_MATERIAL_MAP_NORMAL].texture;         // NOTE: By default, not set
...
bool RL_IsMaterialReady(RlMaterial material)
{
...
void RL_UnloadMaterial(RlMaterial material)
{
...
void RL_SetMaterialTexture(RlMaterial *material, int mapType, RlTexture2D texture)
{
    material->maps[mapType].texture = texture;
}

// Set the material for a mesh
void RL_SetModelMeshMaterial(RlModel *model, int meshId, int materialId)
{
    if (meshId >= model->meshCount) TRACELOG(RL_LOG_WARNING, "MESH: Id greater than mesh count");
    else if (materialId >= model->materialCount) TRACELOG(RL_LOG_WARNING, "MATERIAL: Id greater than material count");
    else  model->meshMaterial[meshId] = materialId;
}

// Load model animations from file
RlModelAnimation *RL_LoadModelAnimations(const char *fileName, unsigned int *animCount)
{
    RlModelAnimation *animations = NULL;
...
void RL_UpdateModelAnimation(RlModel model, RlModelAnimation anim, int frame)
{
    if ((anim.frameCount > 0) && (anim.bones != NULL) && (anim.framePoses != NULL))
    {
        ...
        for (int m = 0; m < model.meshCount; m++)
        {
            ...
            {
                TRACELOG(RL_LOG_WARNING, "MODEL: RL_UpdateModelAnimation(): RlMesh %i has no connection to bones", m);
                continue;
            }

            bool updated = false;           // Flag to check when anim vertex information is updated
            RlVector3 animVertex = { 0 };
            RlVector3 animNormal = { 0 };

            RlVector3 inTranslation = { 0 };
            RlQuaternion inRotation = { 0 };
            // RlVector3 inScale = { 0 };

            RlVector3 outTranslation = { 0 };
            RlQuaternion outRotation = { 0 };
            RlVector3 outScale = { 0 };
            ...
            for (int vCounter = 0; vCounter < vValues; vCounter += 3)
            {
                ...
                for (int j = 0; j < 4; j++, boneCounter++)
                {
                    ...
                    animVertex = (RlVector3){ mesh.vertices[vCounter], mesh.vertices[vCounter + 1], mesh.vertices[vCounter + 2] };
                    ...
                    if (mesh.normals != NULL)
                    {
                        animNormal = (RlVector3){ mesh.normals[vCounter], mesh.normals[vCounter + 1], mesh.normals[vCounter + 2] };
...
void RL_UnloadModelAnimations(RlModelAnimation *animations, unsigned int count)
{
    for (unsigned int i = 0; i < count; i++) RL_UnloadModelAnimation(animations[i]);
    RL_FREE(animations);
}

// Unload animation data
void RL_UnloadModelAnimation(RlModelAnimation anim)
{
...
bool RL_IsModelAnimationValid(RlModel model, RlModelAnimation anim)
{
    int result = true;
...
RlMesh RL_GenMeshPoly(int sides, float radius)
{
    RlMesh mesh = { 0 };
    ...
    // Vertices definition
    RlVector3 *vertices = (RlVector3 *)RL_MALLOC(vertexCount*sizeof(RlVector3));

    float d = 0.0f, dStep = 360.0f/sides;
    for (int v = 0; v < vertexCount; v += 3)
    {
        vertices[v] = (RlVector3){ 0.0f, 0.0f, 0.0f };
        vertices[v + 1] = (RlVector3){ sinf(DEG2RAD*d)*radius, 0.0f, cosf(DEG2RAD*d)*radius };
        vertices[v + 2] = (RlVector3){sinf(DEG2RAD*(d+dStep))*radius, 0.0f, cosf(DEG2RAD*(d+dStep))*radius };
        d += dStep;
    }

    // Normals definition
    RlVector3 *normals = (RlVector3 *)RL_MALLOC(vertexCount*sizeof(RlVector3));
    for (int n = 0; n < vertexCount; n++) normals[n] = (RlVector3){ 0.0f, 1.0f, 0.0f };   // RlVector3.up;

    // TexCoords definition
    RlVector2 *texcoords = (RlVector2 *)RL_MALLOC(vertexCount*sizeof(RlVector2));
    for (int n = 0; n < vertexCount; n++) texcoords[n] = (RlVector2){ 0.0f, 0.0f };
    ...
    // RlMesh vertices position array
...
RlMesh RL_GenMeshPlane(float width, float length, int resX, int resZ)
{
    RlMesh mesh = { 0 };
...
#define CUSTOM_MESH_GEN_PLANE
    ...
    for (int z = 0; z < resZ; z++)
    {
        ...
        for (int x = 0; x < resX; x++)
        {
            ...
            vertices[x + z*resX] = (RlVector3){ xPos, 0.0f, zPos };
        }
    }
    ...
    for (int v = 0; v < resZ; v++)
    {
        ...
        {
            texcoords[u + v*resX] = (RlVector2){ (float)u/(resX - 1), (float)v/(resZ - 1) };
        }
    }
    ...
    // RlMesh indices array initialization
...
RlMesh RL_GenMeshCube(float width, float height, float length)
{
    RlMesh mesh = { 0 };
...
RlMesh RL_GenMeshSphere(float radius, int rings, int slices)
{
    RlMesh mesh = { 0 };
    ...
    else TRACELOG(RL_LOG_WARNING, "MESH: Failed to generate mesh: sphere");

    return mesh;
}

// Generate hemisphere mesh (half sphere, no bottom cap)
RlMesh RL_GenMeshHemiSphere(float radius, int rings, int slices)
{
    RlMesh mesh = { 0 };
    ...
    else TRACELOG(RL_LOG_WARNING, "MESH: Failed to generate mesh: hemisphere");

    return mesh;
}

// Generate cylinder mesh
RlMesh RL_GenMeshCylinder(float radius, float height, int slices)
{
    RlMesh mesh = { 0 };
    ...
    else TRACELOG(RL_LOG_WARNING, "MESH: Failed to generate mesh: cylinder");

    return mesh;
}

// Generate cone/pyramid mesh
RlMesh RL_GenMeshCone(float radius, float height, int slices)
{
    RlMesh mesh = { 0 };
    ...
    else TRACELOG(RL_LOG_WARNING, "MESH: Failed to generate mesh: cone");

    return mesh;
}

// Generate torus mesh
RlMesh RL_GenMeshTorus(float radius, float size, int radSeg, int sides)
{
    RlMesh mesh = { 0 };
    ...
    else TRACELOG(RL_LOG_WARNING, "MESH: Failed to generate mesh: torus");

    return mesh;
}

// Generate trefoil knot mesh
RlMesh RL_GenMeshKnot(float radius, float size, int radSeg, int sides)
{
    RlMesh mesh = { 0 };
    ...
    else TRACELOG(RL_LOG_WARNING, "MESH: Failed to generate mesh: knot");
...
RlMesh RL_GenMeshHeightmap(RlImage heightmap, RlVector3 size)
{
    ...
    RlColor *pixels = RL_LoadImageColors(heightmap);
    ...
    RlVector3 scaleFactor = { size.x/(mapX - 1), size.y/255.0f, size.z/(mapZ - 1) };
...
// NOTE: Vertex data is uploaded to GPU
RlMesh RL_GenMeshCubicmap(RlImage cubicmap, RlVector3 cubeSize)
{
    #define COLOR_EQUAL(col1, col2) ((col1.r == col2.r)&&(col1.g == col2.g)&&(col1.b == col2.b)&&(col1.a == col2.a))

    RlMesh mesh = { 0 };

    RlColor *pixels = RL_LoadImageColors(cubicmap);
    ...
    RlVector3 *mapVertices = (RlVector3 *)RL_MALLOC(maxTriangles*3*sizeof(RlVector3));
    RlVector2 *mapTexcoords = (RlVector2 *)RL_MALLOC(maxTriangles*3*sizeof(RlVector2));
    RlVector3 *mapNormals = (RlVector3 *)RL_MALLOC(maxTriangles*3*sizeof(RlVector3));

    // Define the 6 normals of the cube, we will combine them accordingly later...
    RlVector3 n1 = { 1.0f, 0.0f, 0.0f };
    RlVector3 n2 = { -1.0f, 0.0f, 0.0f };
    RlVector3 n3 = { 0.0f, 1.0f, 0.0f };
    RlVector3 n4 = { 0.0f, -1.0f, 0.0f };
    RlVector3 n5 = { 0.0f, 0.0f, -1.0f };
    RlVector3 n6 = { 0.0f, 0.0f, 1.0f };
    ...
    for (int z = 0; z < cubicmap.height; ++z)
    {
        for (int x = 0; x < cubicmap.width; ++x)
        {
            ...
            RlVector3 v1 = { w*(x - 0.5f), h2, h*(z - 0.5f) };
            RlVector3 v2 = { w*(x - 0.5f), h2, h*(z + 0.5f) };
            RlVector3 v3 = { w*(x + 0.5f), h2, h*(z + 0.5f) };
            RlVector3 v4 = { w*(x + 0.5f), h2, h*(z - 0.5f) };
            RlVector3 v5 = { w*(x + 0.5f), 0, h*(z - 0.5f) };
            RlVector3 v6 = { w*(x - 0.5f), 0, h*(z - 0.5f) };
            RlVector3 v7 = { w*(x - 0.5f), 0, h*(z + 0.5f) };
            RlVector3 v8 = { w*(x + 0.5f), 0, h*(z + 0.5f) };
            ...
            if (COLOR_EQUAL(pixels[z*cubicmap.width + x], WHITE))
            {
                ...
                mapTexcoords[tcCounter] = (RlVector2){ topTexUV.x, topTexUV.y };
                mapTexcoords[tcCounter + 1] = (RlVector2){ topTexUV.x, topTexUV.y + topTexUV.height };
                mapTexcoords[tcCounter + 2] = (RlVector2){ topTexUV.x + topTexUV.width, topTexUV.y + topTexUV.height };
                mapTexcoords[tcCounter + 3] = (RlVector2){ topTexUV.x, topTexUV.y };
                mapTexcoords[tcCounter + 4] = (RlVector2){ topTexUV.x + topTexUV.width, topTexUV.y + topTexUV.height };
                mapTexcoords[tcCounter + 5] = (RlVector2){ topTexUV.x + topTexUV.width, topTexUV.y };
                tcCounter += 6;
                ...
                mapTexcoords[tcCounter] = (RlVector2){ bottomTexUV.x + bottomTexUV.width, bottomTexUV.y };
                mapTexcoords[tcCounter + 1] = (RlVector2){ bottomTexUV.x, bottomTexUV.y + bottomTexUV.height };
                mapTexcoords[tcCounter + 2] = (RlVector2){ bottomTexUV.x + bottomTexUV.width, bottomTexUV.y + bottomTexUV.height };
                mapTexcoords[tcCounter + 3] = (RlVector2){ bottomTexUV.x + bottomTexUV.width, bottomTexUV.y };
                mapTexcoords[tcCounter + 4] = (RlVector2){ bottomTexUV.x, bottomTexUV.y };
                mapTexcoords[tcCounter + 5] = (RlVector2){ bottomTexUV.x, bottomTexUV.y + bottomTexUV.height };
                tcCounter += 6;
                ...
                if (((z < cubicmap.height - 1) && COLOR_EQUAL(pixels[(z + 1)*cubicmap.width + x], BLACK)) || (z == cubicmap.height - 1))
                {
                    ...
                    mapTexcoords[tcCounter] = (RlVector2){ frontTexUV.x, frontTexUV.y };
                    mapTexcoords[tcCounter + 1] = (RlVector2){ frontTexUV.x, frontTexUV.y + frontTexUV.height };
                    mapTexcoords[tcCounter + 2] = (RlVector2){ frontTexUV.x + frontTexUV.width, frontTexUV.y };
                    mapTexcoords[tcCounter + 3] = (RlVector2){ frontTexUV.x + frontTexUV.width, frontTexUV.y };
                    mapTexcoords[tcCounter + 4] = (RlVector2){ frontTexUV.x, frontTexUV.y + frontTexUV.height };
                    mapTexcoords[tcCounter + 5] = (RlVector2){ frontTexUV.x + frontTexUV.width, frontTexUV.y + frontTexUV.height };
                    tcCounter += 6;
                }
                ...
                if (((z > 0) && COLOR_EQUAL(pixels[(z - 1)*cubicmap.width + x], BLACK)) || (z == 0))
                {
                    ...
                    mapTexcoords[tcCounter] = (RlVector2){ backTexUV.x + backTexUV.width, backTexUV.y };
                    mapTexcoords[tcCounter + 1] = (RlVector2){ backTexUV.x, backTexUV.y + backTexUV.height };
                    mapTexcoords[tcCounter + 2] = (RlVector2){ backTexUV.x + backTexUV.width, backTexUV.y + backTexUV.height };
                    mapTexcoords[tcCounter + 3] = (RlVector2){ backTexUV.x + backTexUV.width, backTexUV.y };
                    mapTexcoords[tcCounter + 4] = (RlVector2){ backTexUV.x, backTexUV.y };
                    mapTexcoords[tcCounter + 5] = (RlVector2){ backTexUV.x, backTexUV.y + backTexUV.height };
                    tcCounter += 6;
                }
                ...
                if (((x < cubicmap.width - 1) && COLOR_EQUAL(pixels[z*cubicmap.width + (x + 1)], BLACK)) || (x == cubicmap.width - 1))
                {
                    ...
                    mapTexcoords[tcCounter] = (RlVector2){ rightTexUV.x, rightTexUV.y };
                    mapTexcoords[tcCounter + 1] = (RlVector2){ rightTexUV.x, rightTexUV.y + rightTexUV.height };
                    mapTexcoords[tcCounter + 2] = (RlVector2){ rightTexUV.x + rightTexUV.width, rightTexUV.y };
                    mapTexcoords[tcCounter + 3] = (RlVector2){ rightTexUV.x + rightTexUV.width, rightTexUV.y };
                    mapTexcoords[tcCounter + 4] = (RlVector2){ rightTexUV.x, rightTexUV.y + rightTexUV.height };
                    mapTexcoords[tcCounter + 5] = (RlVector2){ rightTexUV.x + rightTexUV.width, rightTexUV.y + rightTexUV.height };
                    tcCounter += 6;
                }
                ...
                if (((x > 0) && COLOR_EQUAL(pixels[z*cubicmap.width + (x - 1)], BLACK)) || (x == 0))
                {
                    ...
                    mapTexcoords[tcCounter] = (RlVector2){ leftTexUV.x, leftTexUV.y };
                    mapTexcoords[tcCounter + 1] = (RlVector2){ leftTexUV.x + leftTexUV.width, leftTexUV.y + leftTexUV.height };
                    mapTexcoords[tcCounter + 2] = (RlVector2){ leftTexUV.x + leftTexUV.width, leftTexUV.y };
                    mapTexcoords[tcCounter + 3] = (RlVector2){ leftTexUV.x, leftTexUV.y };
                    mapTexcoords[tcCounter + 4] = (RlVector2){ leftTexUV.x, leftTexUV.y + leftTexUV.height };
                    mapTexcoords[tcCounter + 5] = (RlVector2){ leftTexUV.x + leftTexUV.width, leftTexUV.y + leftTexUV.height };
                    tcCounter += 6;
                }
            }
            ...
            else if (COLOR_EQUAL(pixels[z*cubicmap.width + x], BLACK))
            {
                ...
                mapTexcoords[tcCounter] = (RlVector2){ topTexUV.x, topTexUV.y };
                mapTexcoords[tcCounter + 1] = (RlVector2){ topTexUV.x + topTexUV.width, topTexUV.y + topTexUV.height };
                mapTexcoords[tcCounter + 2] = (RlVector2){ topTexUV.x, topTexUV.y + topTexUV.height };
                mapTexcoords[tcCounter + 3] = (RlVector2){ topTexUV.x, topTexUV.y };
                mapTexcoords[tcCounter + 4] = (RlVector2){ topTexUV.x + topTexUV.width, topTexUV.y };
                mapTexcoords[tcCounter + 5] = (RlVector2){ topTexUV.x + topTexUV.width, topTexUV.y + topTexUV.height };
                tcCounter += 6;
                ...
                mapTexcoords[tcCounter] = (RlVector2){ bottomTexUV.x + bottomTexUV.width, bottomTexUV.y };
                mapTexcoords[tcCounter + 1] = (RlVector2){ bottomTexUV.x + bottomTexUV.width, bottomTexUV.y + bottomTexUV.height };
                mapTexcoords[tcCounter + 2] = (RlVector2){ bottomTexUV.x, bottomTexUV.y + bottomTexUV.height };
                mapTexcoords[tcCounter + 3] = (RlVector2){ bottomTexUV.x + bottomTexUV.width, bottomTexUV.y };
                mapTexcoords[tcCounter + 4] = (RlVector2){ bottomTexUV.x, bottomTexUV.y + bottomTexUV.height };
                mapTexcoords[tcCounter + 5] = (RlVector2){ bottomTexUV.x, bottomTexUV.y };
                tcCounter += 6;
            }
        }
    }
...
RlBoundingBox RL_GetMeshBoundingBox(RlMesh mesh)
{
    // Get min and max vertex to construct bounds (AABB)
    RlVector3 minVertex = { 0 };
    RlVector3 maxVertex = { 0 };

    if (mesh.vertices != NULL)
    {
        minVertex = (RlVector3){ mesh.vertices[0], mesh.vertices[1], mesh.vertices[2] };
        maxVertex = (RlVector3){ mesh.vertices[0], mesh.vertices[1], mesh.vertices[2] };

        for (int i = 1; i < mesh.vertexCount; i++)
        {
            minVertex = Vector3Min(minVertex, (RlVector3){ mesh.vertices[i*3], mesh.vertices[i*3 + 1], mesh.vertices[i*3 + 2] });
            maxVertex = Vector3Max(maxVertex, (RlVector3){ mesh.vertices[i*3], mesh.vertices[i*3 + 1], mesh.vertices[i*3 + 2] });
        }
    }
...
void RL_GenMeshTangents(RlMesh *mesh)
{
    if ((mesh->vertices == NULL) || (mesh->texcoords == NULL))
    {
        TRACELOG(RL_LOG_WARNING, "MESH: Tangents generation requires texcoord vertex attribute data");
        return;
    }
    ...
    RlVector3 *tan1 = (RlVector3 *)RL_MALLOC(mesh->vertexCount*sizeof(RlVector3));
    RlVector3 *tan2 = (RlVector3 *)RL_MALLOC(mesh->vertexCount*sizeof(RlVector3));

    for (int i = 0; i < mesh->vertexCount; i += 3)
    {
        // Get triangle vertices
        RlVector3 v1 = { mesh->vertices[(i + 0)*3 + 0], mesh->vertices[(i + 0)*3 + 1], mesh->vertices[(i + 0)*3 + 2] };
        RlVector3 v2 = { mesh->vertices[(i + 1)*3 + 0], mesh->vertices[(i + 1)*3 + 1], mesh->vertices[(i + 1)*3 + 2] };
        RlVector3 v3 = { mesh->vertices[(i + 2)*3 + 0], mesh->vertices[(i + 2)*3 + 1], mesh->vertices[(i + 2)*3 + 2] };

        // Get triangle texcoords
        RlVector2 uv1 = { mesh->texcoords[(i + 0)*2 + 0], mesh->texcoords[(i + 0)*2 + 1] };
        RlVector2 uv2 = { mesh->texcoords[(i + 1)*2 + 0], mesh->texcoords[(i + 1)*2 + 1] };
        RlVector2 uv3 = { mesh->texcoords[(i + 2)*2 + 0], mesh->texcoords[(i + 2)*2 + 1] };
        ...
        RlVector3 sdir = { (t2*x1 - t1*x2)*r, (t2*y1 - t1*y2)*r, (t2*z1 - t1*z2)*r };
        RlVector3 tdir = { (s1*x2 - s2*x1)*r, (s1*y2 - s2*y1)*r, (s1*z2 - s2*z1)*r };
    ...
    for (int i = 0; i < mesh->vertexCount; i++)
    {
        RlVector3 normal = { mesh->normals[i*3 + 0], mesh->normals[i*3 + 1], mesh->normals[i*3 + 2] };
        RlVector3 tangent = tan1[i];

        // TODO: Review, not sure if tangent computation is right, just used reference proposed maths...
#if defined(COMPUTE_TANGENTS_METHOD_01)
        RlVector3 tmp = Vector3Subtract(tangent, Vector3Scale(normal, Vector3DotProduct(normal, tangent)));
    ...
    if (mesh->vboId != NULL)
    {
        if (mesh->vboId[RL_SHADER_LOC_VERTEX_TANGENT] != 0)
        {
            ...
            rlUpdateVertexBuffer(mesh->vboId[RL_SHADER_LOC_VERTEX_TANGENT], mesh->tangents, mesh->vertexCount*4*sizeof(float), 0);
        }
        else
        {
            // Load a new tangent attributes buffer
            mesh->vboId[RL_SHADER_LOC_VERTEX_TANGENT] = rlLoadVertexBuffer(mesh->tangents, mesh->vertexCount*4*sizeof(float), false);
        }
    ...
    TRACELOG(RL_LOG_INFO, "MESH: Tangents data computed and uploaded for provided mesh");
}

// Draw a model (with texture if set)
void RL_DrawModel(RlModel model, RlVector3 position, float scale, RlColor tint)
{
    RlVector3 vScale = { scale, scale, scale };
    RlVector3 rotationAxis = { 0.0f, 1.0f, 0.0f };

    RL_DrawModelEx(model, position, rotationAxis, 0.0f, vScale, tint);
}

// Draw a model with extended parameters
void RL_DrawModelEx(RlModel model, RlVector3 position, RlVector3 rotationAxis, float rotationAngle, RlVector3 scale, RlColor tint)
{
    // Calculate transformation matrix from function parameters
    // Get transform matrix (rotation -> scale -> translation)
    RlMatrix matScale = MatrixScale(scale.x, scale.y, scale.z);
    RlMatrix matRotation = MatrixRotate(rotationAxis, rotationAngle*DEG2RAD);
    RlMatrix matTranslation = MatrixTranslate(position.x, position.y, position.z);

    RlMatrix matTransform = MatrixMultiply(MatrixMultiply(matScale, matRotation), matTranslation);
...
void RL_DrawModelWires(RlModel model, RlVector3 position, float scale, RlColor tint)
{
    rlEnableWireMode();
...
void RL_DrawModelWiresEx(RlModel model, RlVector3 position, RlVector3 rotationAxis, float rotationAngle, RlVector3 scale, RlColor tint)
{
    rlEnableWireMode();
...
// Draw a billboard
void RL_DrawBillboard(RlCamera camera, RlTexture2D texture, RlVector3 position, float size, RlColor tint)
{
    RlRectangle source = { 0.0f, 0.0f, (float)texture.width, (float)texture.height };

    RL_DrawBillboardRec(camera, texture, source, position, (RlVector2){ size, size }, tint);
}

// Draw a billboard (part of a texture defined by a rectangle)
void RL_DrawBillboardRec(RlCamera camera, RlTexture2D texture, RlRectangle source, RlVector3 position, RlVector2 size, RlColor tint)
{
    // NOTE: Billboard locked on axis-Y
    RlVector3 up = { 0.0f, 1.0f, 0.0f };

    RL_DrawBillboardPro(camera, texture, source, position, up, size, Vector2Zero(), 0.0f, tint);
}

void RL_DrawBillboardPro(RlCamera camera, RlTexture2D texture, RlRectangle source, RlVector3 position, RlVector3 up, RlVector2 size, RlVector2 origin, float rotation, RlColor tint)
{
    // NOTE: Billboard size will maintain source rectangle aspect ratio, size will represent billboard width
    RlVector2 sizeRatio = { size.x*fabsf((float)source.width/source.height), size.y };

    RlMatrix matView = MatrixLookAt(camera.position, camera.target, camera.up);

    RlVector3 right = { matView.m0, matView.m4, matView.m8 };
    //RlVector3 up = { matView.m1, matView.m5, matView.m9 };

    RlVector3 rightScaled = Vector3Scale(right, sizeRatio.x/2);
    RlVector3 upScaled = Vector3Scale(up, sizeRatio.y/2);

    RlVector3 p1 = Vector3Add(rightScaled, upScaled);
    RlVector3 p2 = Vector3Subtract(rightScaled, upScaled);

    RlVector3 topLeft = Vector3Scale(p2, -1);
    RlVector3 topRight = p1;
    RlVector3 bottomRight = p2;
    RlVector3 bottomLeft = Vector3Scale(p1, -1);
...
void RL_DrawBoundingBox(RlBoundingBox box, RlColor color)
{
    RlVector3 size = { 0 };
    ...
    RlVector3 center = { box.min.x + size.x/2.0f, box.min.y + size.y/2.0f, box.min.z + size.z/2.0f };

    RL_DrawCubeWires(center, size.x, size.y, size.z, color);
}

// Check collision between two spheres
bool RL_CheckCollisionSpheres(RlVector3 center1, float radius1, RlVector3 center2, float radius2)
{
...
bool RL_CheckCollisionBoxes(RlBoundingBox box1, RlBoundingBox box2)
{
    bool collision = true;
...
bool RL_CheckCollisionBoxSphere(RlBoundingBox box, RlVector3 center, float radius)
{
    bool collision = false;
...
RlRayCollision RL_GetRayCollisionSphere(RlRay ray, RlVector3 center, float radius)
{
    RlRayCollision collision = { 0 };

    RlVector3 raySpherePos = Vector3Subtract(center, ray.position);
...
RlRayCollision RL_GetRayCollisionBox(RlRay ray, RlBoundingBox box)
{
    RlRayCollision collision = { 0 };
...
RlRayCollision RL_GetRayCollisionMesh(RlRay ray, RlMesh mesh, RlMatrix transform)
{
    RlRayCollision collision = { 0 };
    ...
    if (mesh.vertices != NULL)
    {
        ...
        {
            RlVector3 a, b, c;
            RlVector3* vertdata = (RlVector3*)mesh.vertices;
            ...
            RlRayCollision triHitInfo = RL_GetRayCollisionTriangle(ray, a, b, c);
...
RlRayCollision RL_GetRayCollisionTriangle(RlRay ray, RlVector3 p1, RlVector3 p2, RlVector3 p3)
{
    ...
    if (t > EPSILON)
    {
        // RlRay hit, get hit point and normal
        collision.hit = true;
...
// NOTE: The points are expected to be in counter-clockwise winding
RlRayCollision RL_GetRayCollisionQuad(RlRay ray, RlVector3 p1, RlVector3 p2, RlVector3 p3, RlVector3 p4)
{
    RlRayCollision collision = { 0 };
...
static void BuildPoseFromParentJoints(RlBoneInfo *bones, int boneCount, RlTransform *transforms)
{
    for (int i = 0; i < boneCount; i++)
    {
        if (bones[i].parent >= 0)
        {
            if (bones[i].parent > i)
            {
                TRACELOG(RL_LOG_WARNING, "Assumes bones are toplogically sorted, but bone %d has parent %d. Skipping.", i, bones[i].parent);
                continue;
            }
...
static RlModel LoadOBJ(const char *fileName)
{
    ...
    if (fileText != NULL)
    {
        ...
        strcpy(currentDir, RL_GetWorkingDirectory());
        const char *workingDir = RL_GetDirectoryPath(fileName);
        if (CHDIR(workingDir) != 0)
        {
            TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Failed to change working directory", workingDir);
        }

        unsigned int flags = TINYOBJ_FLAG_TRIANGULATE;
        int ret = tinyobj_parse_obj(&attrib, &meshes, &meshCount, &materials, &materialCount, fileText, dataSize, flags);

        if (ret != TINYOBJ_SUCCESS) TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Failed to load OBJ data", fileName);
        else TRACELOG(RL_LOG_INFO, "MODEL: [%s] OBJ data loaded successfully: %i meshes/%i materials", fileName, meshCount, materialCount);
        ...
        if (materialCount > 0)
        {
            ...
            model.materials = (RlMaterial *)RL_CALLOC(model.materialCount, sizeof(RlMaterial));
            TRACELOG(RL_LOG_INFO, "MODEL: model has %i material meshes", materialCount);
        }
        else
        {
            model.meshCount = 1;
            TRACELOG(RL_LOG_INFO, "MODEL: No materials, putting all meshes in a default material");
        }
        ...
        {
            TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Failed to change working directory", currentDir);
        }
    }
...
#if defined(SUPPORT_FILEFORMAT_IQM)
// Load IQM mesh data
static RlModel LoadIQM(const char *fileName)
{
    #define IQM_MAGIC     "INTERQUAKEMODEL" // IQM file magic number
    #define IQM_VERSION          2          // only IQM version 2 supported

    #define BONE_NAME_LENGTH    32          // RlBoneInfo name string length
    #define MESH_NAME_LENGTH    32          // RlMesh name string length
    #define MATERIAL_NAME_LENGTH 32         // RlMaterial name string length

    unsigned int fileSize = 0;
    unsigned char *fileData = RL_LoadFileData(fileName, &fileSize);
    ...
    */
    ...
    {
        TRACELOG(RL_LOG_WARNING, "MODEL: [%s] IQM file is not a valid model", fileName);
        return model;
    }

    if (iqmHeader->version != IQM_VERSION)
    {
        TRACELOG(RL_LOG_WARNING, "MODEL: [%s] IQM file version not supported (%i)", fileName, iqmHeader->version);
        return model;
    }
    ...
    model.meshes = RL_CALLOC(model.meshCount, sizeof(RlMesh));
    ...
    for (int i = 0; i < model.meshCount; i++)
    {
        ...
        TRACELOG(RL_LOG_DEBUG, "MODEL: [%s] mesh name (%s), material (%s)", fileName, name, material);
    ...
    model.bones = RL_MALLOC(iqmHeader->num_joints*sizeof(RlBoneInfo));
    model.bindPose = RL_MALLOC(iqmHeader->num_joints*sizeof(RlTransform));
...
static RlModelAnimation *LoadModelAnimationsIQM(const char *fileName, unsigned int *animCount)
{
    #define IQM_MAGIC       "INTERQUAKEMODEL"   // IQM file magic number
    #define IQM_VERSION     2                   // only IQM version 2 supported
    ...
    *animCount = iqmHeader->num_anims;
    ...
    RlModelAnimation *animations = RL_MALLOC(iqmHeader->num_anims*sizeof(RlModelAnimation));
    ...
    for (unsigned int a = 0; a < iqmHeader->num_anims; a++)
    {
        ...
        animations[a].bones = RL_MALLOC(iqmHeader->num_poses*sizeof(RlBoneInfo));
        animations[a].framePoses = RL_MALLOC(anim[a].num_frames*sizeof(RlTransform *));
        ...
        for (unsigned int j = 0; j < anim[a].num_frames; j++) animations[a].framePoses[j] = RL_MALLOC(iqmHeader->num_poses*sizeof(RlTransform));
...
static RlImage LoadImageFromCgltfImage(cgltf_image *cgltfImage, const char *texPath)
{
    RlImage image = { 0 };
    ...
    if (cgltfImage->uri != NULL)     // Check if image data is provided as an uri (base64 or path)
    {
            ...
            (cgltfImage->uri[4] == ':'))     // Check if image is provided as base64 text data
        {
            ...
            if (cgltfImage->uri[i] == 0) TRACELOG(RL_LOG_WARNING, "IMAGE: glTF data URI is not a valid image");
            else
            {
    ...
    else if (cgltfImage->buffer_view->buffer->data != NULL)    // Check if image is provided as data buffer
    {
        ...
        else TRACELOG(RL_LOG_WARNING, "MODEL: glTF image data MIME type not recognized", RL_TextFormat("%s/%s", texPath, cgltfImage->uri));
...
static RlBoneInfo *LoadBoneInfoGLTF(cgltf_skin skin, int *boneCount)
{
    *boneCount = (int)skin.joints_count;
    RlBoneInfo *bones = RL_MALLOC(skin.joints_count*sizeof(RlBoneInfo));
...
static RlModel LoadGLTF(const char *fileName)
{
    /*********************************************************************************************
    ...
    ***********************************************************************************************/
    ...
    if (result == cgltf_result_success)
    {
        if (data->file_type == cgltf_file_type_glb) TRACELOG(RL_LOG_INFO, "MODEL: [%s] RlModel basic data (glb) loaded successfully", fileName);
        else if (data->file_type == cgltf_file_type_gltf) TRACELOG(RL_LOG_INFO, "MODEL: [%s] RlModel basic data (glTF) loaded successfully", fileName);
        else TRACELOG(RL_LOG_WARNING, "MODEL: [%s] RlModel format not recognized", fileName);

        TRACELOG(RL_LOG_INFO, "    > Meshes count: %i", data->meshes_count);
        TRACELOG(RL_LOG_INFO, "    > Materials count: %i (+1 default)", data->materials_count);
        TRACELOG(RL_LOG_DEBUG, "    > Buffers count: %i", data->buffers_count);
        TRACELOG(RL_LOG_DEBUG, "    > Images count: %i", data->images_count);
        TRACELOG(RL_LOG_DEBUG, "    > Textures count: %i", data->textures_count);

        // Force reading data buffers (fills buffer_view->buffer->data)
        // NOTE: If an uri is defined to base64 data or external path, it's automatically loaded
        result = cgltf_load_buffers(&options, data, fileName);
        if (result != cgltf_result_success) TRACELOG(RL_LOG_INFO, "MODEL: [%s] Failed to load mesh/material buffers", fileName);
        ...
        model.materials = RL_CALLOC(model.materialCount, sizeof(RlMaterial));
        ...
        for (unsigned int i = 0, j = 1; i < data->materials_count; i++, j++)
        {
            ...
            if (data->materials[i].has_pbr_metallic_roughness)
            {
                ...
                {
                    RlImage imAlbedo = LoadImageFromCgltfImage(data->materials[i].pbr_metallic_roughness.base_color_texture.texture->image, texPath);
                    if (imAlbedo.data != NULL)
                    {
                        model.materials[j].maps[RL_MATERIAL_MAP_ALBEDO].texture = RL_LoadTextureFromImage(imAlbedo);
                        RL_UnloadImage(imAlbedo);
                    }
                }
                // Load base color factor (tint)
                model.materials[j].maps[RL_MATERIAL_MAP_ALBEDO].color.r = (unsigned char)(data->materials[i].pbr_metallic_roughness.base_color_factor[0]*255);
                model.materials[j].maps[RL_MATERIAL_MAP_ALBEDO].color.g = (unsigned char)(data->materials[i].pbr_metallic_roughness.base_color_factor[1]*255);
                model.materials[j].maps[RL_MATERIAL_MAP_ALBEDO].color.b = (unsigned char)(data->materials[i].pbr_metallic_roughness.base_color_factor[2]*255);
                model.materials[j].maps[RL_MATERIAL_MAP_ALBEDO].color.a = (unsigned char)(data->materials[i].pbr_metallic_roughness.base_color_factor[3]*255);

                // Load metallic/roughness texture
                if (data->materials[i].pbr_metallic_roughness.metallic_roughness_texture.texture)
                {
                    RlImage imMetallicRoughness = LoadImageFromCgltfImage(data->materials[i].pbr_metallic_roughness.metallic_roughness_texture.texture->image, texPath);
                    if (imMetallicRoughness.data != NULL)
                    {
                        model.materials[j].maps[RL_MATERIAL_MAP_ROUGHNESS].texture = RL_LoadTextureFromImage(imMetallicRoughness);
                        RL_UnloadImage(imMetallicRoughness);
                    }

                    // Load metallic/roughness material properties
                    float roughness = data->materials[i].pbr_metallic_roughness.roughness_factor;
                    model.materials[j].maps[RL_MATERIAL_MAP_ROUGHNESS].value = roughness;

                    float metallic = data->materials[i].pbr_metallic_roughness.metallic_factor;
                    model.materials[j].maps[RL_MATERIAL_MAP_METALNESS].value = metallic;
                }

                // Load normal texture
                if (data->materials[i].normal_texture.texture)
                {
                    RlImage imNormal = LoadImageFromCgltfImage(data->materials[i].normal_texture.texture->image, texPath);
                    if (imNormal.data != NULL)
                    {
                        model.materials[j].maps[RL_MATERIAL_MAP_NORMAL].texture = RL_LoadTextureFromImage(imNormal);
                ...
                {
                    RlImage imOcclusion = LoadImageFromCgltfImage(data->materials[i].occlusion_texture.texture->image, texPath);
                    if (imOcclusion.data != NULL)
                    {
                        model.materials[j].maps[RL_MATERIAL_MAP_OCCLUSION].texture = RL_LoadTextureFromImage(imOcclusion);
                ...
                {
                    RlImage imEmissive = LoadImageFromCgltfImage(data->materials[i].emissive_texture.texture->image, texPath);
                    if (imEmissive.data != NULL)
                    {
                        model.materials[j].maps[RL_MATERIAL_MAP_EMISSION].texture = RL_LoadTextureFromImage(imEmissive);
                        RL_UnloadImage(imEmissive);
                    }

                    // Load emissive color factor
                    model.materials[j].maps[RL_MATERIAL_MAP_EMISSION].color.r = (unsigned char)(data->materials[i].emissive_factor[0]*255);
                    model.materials[j].maps[RL_MATERIAL_MAP_EMISSION].color.g = (unsigned char)(data->materials[i].emissive_factor[1]*255);
                    model.materials[j].maps[RL_MATERIAL_MAP_EMISSION].color.b = (unsigned char)(data->materials[i].emissive_factor[2]*255);
                    model.materials[j].maps[RL_MATERIAL_MAP_EMISSION].color.a = 255;
                }
            }
            ...
            for (unsigned int p = 0; p < data->meshes[i].primitives_count; p++)
            {
                ...
                for (unsigned int j = 0; j < data->meshes[i].primitives[p].attributes_count; j++)
                {
                    ...
                    if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_position)      // POSITION
                    {
                        ...
                        else TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Vertices attribute data format not supported, use vec3 float", fileName);
                    }
                    else if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_normal)   // NORMAL
                    {
                        ...
                        else TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Normal attribute data format not supported, use vec3 float", fileName);
                    }
                    else if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_tangent)   // TANGENT
                    {
                        ...
                        else TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Tangent attribute data format not supported, use vec4 float", fileName);
                    }
                    else if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_texcoord) // TEXCOORD_0
                    {
                        ...
                        else TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Texcoords attribute data format not supported, use vec2 float", fileName);
                    }
                    else if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_color)    // COLOR_0
                    {
                        ...
                        else TRACELOG(RL_LOG_WARNING, "MODEL: [%s] RlColor attribute data format not supported", fileName);
                    }
                ...
                if (data->meshes[i].primitives[p].indices != NULL)
                {
                    ...
                    else if (attribute->component_type == cgltf_component_type_r_32u)
                    {
                        ...
                        TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Indices data converted from u32 to u16, possible loss of data", fileName);

                        RL_FREE(temp);
                    }
                    else TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Indices data format not supported, use u16", fileName);
                }
        ...
        if (data->skins_count == 1)
        {
            ...
            model.bindPose = RL_MALLOC(model.boneCount*sizeof(RlTransform));
        ...
        {
            TRACELOG(RL_LOG_ERROR, "MODEL: [%s] can only load one skin (armature) per model, but gltf skins_count == %i", fileName, data->skins_count);
        }
            ...
            for (unsigned int p = 0; p < data->meshes[i].primitives_count; p++)
            {
                ...
                for (unsigned int j = 0; j < data->meshes[i].primitives[p].attributes_count; j++)
                {
                    ...
                    if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_joints)        // JOINTS_n (vec4: 4 bones max per vertex / u8, u16)
                    {
                        ...
                        else TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Joint attribute data format not supported, use vec4 u8", fileName);
                    }
                    else if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_weights)  // WEIGHTS_n (vec4 / u8, u16, f32)
                    {
                        ...
                        else TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Joint weight attribute data format not supported, use vec4 float", fileName);
                    }
                }
    ...
    else TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Failed to load glTF data", fileName);
...
static bool GetPoseAtTimeGLTF(cgltf_accessor *input, cgltf_accessor *output, float time, void *data)
{
    ...
    if (output->type == cgltf_type_vec3)
    {
        ...
        RlVector3 v1 = {tmp[0], tmp[1], tmp[2]};
        cgltf_accessor_read_float(output, keyframe+1, tmp, 3);
        RlVector3 v2 = {tmp[0], tmp[1], tmp[2]};
        RlVector3 *r = data;
    ...
    else if (output->type == cgltf_type_vec4)
    {
        ...
        RlVector4 v1 = {tmp[0], tmp[1], tmp[2], tmp[3]};
        cgltf_accessor_read_float(output, keyframe+1, tmp, 4);
        RlVector4 v2 = {tmp[0], tmp[1], tmp[2], tmp[3]};
        RlVector4 *r = data;
...
static RlModelAnimation *LoadModelAnimationsGLTF(const char *fileName, unsigned int *animCount)
{
    // glTF file loading
    unsigned int dataSize = 0;
    unsigned char *fileData = RL_LoadFileData(fileName, &dataSize);
    ...
    if (result != cgltf_result_success)
    {
        TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Failed to load glTF data", fileName);
        *animCount = 0;
        return NULL;
    }

    result = cgltf_load_buffers(&options, data, fileName);
    if (result != cgltf_result_success) TRACELOG(RL_LOG_INFO, "MODEL: [%s] Failed to load animation buffers", fileName);
    ...
    if (result == cgltf_result_success)
    {
        if (data->skins_count == 1)
        {
            ...
            *animCount = (int)data->animations_count;
            animations = RL_MALLOC(data->animations_count*sizeof(RlModelAnimation));
            ...
            for (unsigned int i = 0; i < data->animations_count; i++)
            {
                ...
                for (unsigned int j = 0; j < animData.channels_count; j++)
                {
                    ...
                    if (animData.channels[j].sampler->interpolation == cgltf_interpolation_type_linear)
                    {
                        ...
                        {
                            TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Unsupported target_path on channel %d's sampler for animation %d. Skipping.", fileName, j, i);
                        }
                    }
                    else TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Only linear interpolation curves are supported for GLTF animation.", fileName);
                    ...
                    if (!r)
                    {
                        TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Failed to load input time", fileName);
                        continue;
                    }
                ...
                animations[i].framePoses = RL_MALLOC(animations[i].frameCount*sizeof(RlTransform *));

                for (int j = 0; j < animations[i].frameCount; j++)
                {
                    animations[i].framePoses[j] = RL_MALLOC(animations[i].boneCount*sizeof(RlTransform));
                    float time = ((float) j*GLTF_ANIMDELAY)/1000.0f;

                    for (int k = 0; k < animations[i].boneCount; k++)
                    {
                        RlVector3 translation = {0, 0, 0};
                        RlQuaternion rotation = {0, 0, 0, 1};
                        RlVector3 scale = {1, 1, 1};
                        ...
                        if (boneChannels[k].translate)
                        {
                            ...
                            {
                                TRACELOG(RL_LOG_INFO, "MODEL: [%s] Failed to load translate pose data for bone %s", fileName, animations[i].bones[k].name);
                            }
                        }
                        ...
                        if (boneChannels[k].rotate)
                        {
                            ...
                            {
                                TRACELOG(RL_LOG_INFO, "MODEL: [%s] Failed to load rotate pose data for bone %s", fileName, animations[i].bones[k].name);
                            }
                        }
                        ...
                        if (boneChannels[k].scale)
                        {
                            ...
                            {
                                TRACELOG(RL_LOG_INFO, "MODEL: [%s] Failed to load scale pose data for bone %s", fileName, animations[i].bones[k].name);
                            }
                        }

                        animations[i].framePoses[j][k] = (RlTransform){
                ...
                TRACELOG(RL_LOG_INFO, "MODEL: [%s] Loaded animation: %s (%d frames, %fs)", fileName, animData.name, animations[i].frameCount, animDuration);
                RL_FREE(boneChannels);
            }
        }
        else TRACELOG(RL_LOG_ERROR, "MODEL: [%s] expected exactly one skin to load animation data from, but found %i", fileName, data->skins_count);
...
static RlModel LoadVOX(const char *fileName)
{
    RlModel model = { 0 };
    ...
    fileData = RL_LoadFileData(fileName, &fileSize);
    if (fileData == 0)
    {
        TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Failed to load VOX file", fileName);
        return model;
    }
    ...
    if (ret != VOX_SUCCESS)
    {
        ...
        TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Failed to load VOX data", fileName);
        return model;
    }
    else
    {
        ...
        TRACELOG(RL_LOG_INFO, "MODEL: [%s] VOX data loaded successfully : %i vertices/%i meshes", fileName, nbvertices, meshescount);
    }
    ...
    RlVector3 *pvertices = (RlVector3 *)voxarray.vertices.array;
    RlColor *pcolors = (RlColor *)voxarray.colors.array;
    ...
    for (int i = 0; i < meshescount; i++)
    {
        ...
        // Copy colors
        size = pmesh->vertexCount*sizeof(RlColor);
...
unsigned char *m3d_loaderhook(char *fn, unsigned int *len) { return RL_LoadFileData((const char *)fn, len); }
void m3d_freehook(void *data) { RL_UnloadFileData((unsigned char *)data); }

// Load M3D mesh data
static RlModel LoadM3D(const char *fileName)
{
    RlModel model = { 0 };
    ...
    unsigned char *fileData = RL_LoadFileData(fileName, &bytesRead);
    int i, j, k, l, n, mi = -2;
    ...
    if (fileData != NULL)
    {
        ...
        {
            TRACELOG(RL_LOG_WARNING, "MODEL: [%s] Failed to load M3D data, error code %d", fileName, m3d ? m3d->errcode : -2);
            if (m3d) m3d_free(m3d);
            RL_UnloadFileData(fileData);
            return model;
        }
        else TRACELOG(RL_LOG_INFO, "MODEL: [%s] M3D data loaded successfully: %i faces/%i materials", fileName, m3d->numface, m3d->nummaterial);
        ...
        if (m3d->nummaterial > 0)
        {
            ...
            TRACELOG(RL_LOG_INFO, "MODEL: model has %i material meshes", model.materialCount);
        }
        else
        {
        ...
        model.meshes = (RlMesh *)RL_CALLOC(model.meshCount, sizeof(RlMesh));
        model.meshMaterial = (int *)RL_CALLOC(model.meshCount, sizeof(int));
        model.materials = (RlMaterial *)RL_CALLOC(model.materialCount + 1, sizeof(RlMaterial));
        ...
        for (i = l = 0, k = -1; i < (int)m3d->numface; i++, l++)
        {
            ...
            if (mi != m3d->face[i].materialid)
            {
                ...
                {
                    model.meshCount++;
                    model.meshes = (RlMesh *)RL_REALLOC(model.meshes, model.meshCount*sizeof(RlMesh));
                    memset(&model.meshes[model.meshCount - 1], 0, sizeof(RlMesh));
        ...
        for (i = 0; i < (int)m3d->nummaterial; i++)
        {
            ...
            for (j = 0; j < m3d->material[i].numprop; j++)
            {
                ...
                switch (prop->type)
                {
                    ...
                    case m3dp_Ke:
                    {
                        memcpy(&model.materials[i + 1].maps[RL_MATERIAL_MAP_EMISSION].color, &prop->value.color, 4);
                        model.materials[i + 1].maps[RL_MATERIAL_MAP_EMISSION].value = 0.0f;
                    } break;
                    case m3dp_Pm:
                    {
                        model.materials[i + 1].maps[RL_MATERIAL_MAP_METALNESS].value = prop->value.fnum;
                    } break;
                    case m3dp_Pr:
                    {
                        model.materials[i + 1].maps[RL_MATERIAL_MAP_ROUGHNESS].value = prop->value.fnum;
                    } break;
                    case m3dp_Ps:
                    {
                        model.materials[i + 1].maps[RL_MATERIAL_MAP_NORMAL].color = WHITE;
                        model.materials[i + 1].maps[RL_MATERIAL_MAP_NORMAL].value = prop->value.fnum;
                    } break;
                    default:
                    {
                        if (prop->type >= 128)
                        {
                            ...
                            image.mipmaps = 1;
                            image.format = (m3d->texture[prop->value.textureid].f == 4)? RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8 :
                                           ((m3d->texture[prop->value.textureid].f == 3)? RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8 :
                                           ((m3d->texture[prop->value.textureid].f == 2)? RL_PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA : RL_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE));
                            ...
                            switch (prop->type)
                            {
                                ...
                                case m3dp_map_Ke: model.materials[i + 1].maps[RL_MATERIAL_MAP_EMISSION].texture = RL_LoadTextureFromImage(image); break;
                                case m3dp_map_Km: model.materials[i + 1].maps[RL_MATERIAL_MAP_NORMAL].texture = RL_LoadTextureFromImage(image); break;
                                case m3dp_map_Ka: model.materials[i + 1].maps[RL_MATERIAL_MAP_OCCLUSION].texture = RL_LoadTextureFromImage(image); break;
                                case m3dp_map_Pm: model.materials[i + 1].maps[RL_MATERIAL_MAP_ROUGHNESS].texture = RL_LoadTextureFromImage(image); break;
                                default: break;
                            }
                        }
                    } break;
        ...
        if (m3d->numbone)
        {
            ...
            model.bones = RL_CALLOC(model.boneCount, sizeof(RlBoneInfo));
            model.bindPose = RL_CALLOC(model.boneCount, sizeof(RlTransform));
...
static RlModelAnimation *LoadModelAnimationsM3D(const char *fileName, unsigned int *animCount)
{
    m3d_t *m3d = NULL;
    unsigned int bytesRead = 0;
    unsigned char *fileData = RL_LoadFileData(fileName, &bytesRead);
    ...
    *animCount = 0;
    ...
    if (fileData != NULL)
    {
        ...
        else TRACELOG(RL_LOG_INFO, "MODEL: [%s] M3D data loaded successfully: %i animations, %i bones, %i skins", fileName,
        ...
        animations = RL_MALLOC(m3d->numaction*sizeof(RlModelAnimation));
        *animCount = m3d->numaction;
        ...
        for (unsigned int a = 0; a < m3d->numaction; a++)
        {
            ...
            animations[a].bones = RL_MALLOC((m3d->numbone + 1)*sizeof(RlBoneInfo));
            animations[a].framePoses = RL_MALLOC(animations[a].frameCount*sizeof(RlTransform *));
            // strncpy(animations[a].name, m3d->action[a].name, sizeof(animations[a].name));
            TRACELOG(RL_LOG_INFO, "MODEL: [%s] animation #%i: %i msec, %i frames", fileName, a, m3d->action[a].durationmsec, animations[a].frameCount);
            ...
            {
                animations[a].framePoses[i] = RL_MALLOC((m3d->numbone + 1)*sizeof(RlTransform));
